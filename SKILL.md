...
- level of detail, especially deadlines and authorities.
...
3. Extract each legally meaningful item into this schema:
   - **actor**;
...
5. Never invent a missing deadline. Label it “规则未规定／未设固定期限” and cite the provision that creates the step when possible.
...
2. Check that every deadline shows its triggering event and unit.
3. Check branches for missing outcomes, especially dismissal, default, tolling, appeal, remand, settlement, referral, and enforcement.
4. Validate SVG structure with:

   `python3 scripts/validate_svg.py <file-or-directory>`

5. Render each SVG to a raster preview and inspect the full image for clipping, overflow, connector collisions, unreadable citations, and inconsistent spacing.
6. Keep the SVG as the editable source. A PNG is only a preview.
7. Deliver each file with a descriptive name and state the governing source version or effective date.
