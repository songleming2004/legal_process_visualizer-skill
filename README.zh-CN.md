...
- **期间关系图**：区分时点、法定或裁判计算期间、持续事实状态、时效中断、中止、期间停止计算及新旧法过渡窗口。
...
- 在端点对齐或期间比较是分析重点时，可用字体式花括号展示嵌套、重叠期间，同时通过标题、颜色和依据区分法律期间、事实状态与估算区间。
...
![用可编辑字体式花括号比较法律期间、事实状态及其法律相关重叠区间](assets/readme-temporal-brace-example.svg)

这个完全虚构的示例说明了花括号比普通色带更合适的一种情形：分析重点是精确比较各期间的起止端点。原诉讼时效期间、持续事实障碍和法律相关重叠区间分别保留自己的端点、性质标签和占位依据；虚线把每个重要端点投影到同一时间轴，颜色与文字则进一步区分法律期间、事实状态和法院计算。
...
> 制作一张可编辑时间线，展示原诉讼时效期间、重新起算日、持续事实障碍、具有法律意义的重叠部分，以及新旧法过渡后法院认定的期间。每个期间卡片内写明相应依据。

**会得到什么**

一张带依据的期间关系图。每个时间对象都保留独立端点；在需要精确对齐或比较区间时使用字体式花括号。花括号的整体高度和可见字重可以分别调节，不会因拉长或减细而扭曲两端、肩部或中央收束。
